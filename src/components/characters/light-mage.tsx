...
// Light Mage specific animations and configurations
export const LIGHT_MAGE_ANIMATIONS = {
  ...
  Light_ball: { frames: 7, frameRate: 14 },
  Light_charge: { frames: 12, frameRate: 24 },
  Hurt: { frames: 3, frameRate: 6 },
  Dead: { frames: 5, frameRate: 10 }
} as const;

export type LightMageAnimationType = keyof typeof LIGHT_MAGE_ANIMATIONS;

interface LightMageProps {
  animation?: LightMageAnimationType;
  scale?: number;
  onAnimationComplete?: () => void;
  className?: string;
}

export function LightMage({
  animation = 'Idle',
  scale = 1,
  onAnimationComplete,
  className = ''
  ...
  const getSpritePath = () => {
    return `/sprites/mages/light-mage/${animation}.png`;
  };
  ...
  const loadSpriteSheet = () => {
    ...
    image.onload = () => {
      setIsLoading(false);
      const animConfig = LIGHT_MAGE_ANIMATIONS[animation];
      requestAnimationFrame(() => startAnimation(image, animConfig));
    };

    image.onerror = () => {
      console.error('Failed to load Light Mage sprite:', spritePath);
