...
// Skeleton specific animations and configurations
export const SKELETON_ANIMATIONS = {
  ...
  Special_attack: { frames: 5, frameRate: 10 },
  Hurt: { frames: 3, frameRate: 6 },
  Dead: { frames: 3, frameRate: 6 }
} as const;

export type SkeletonAnimationType = keyof typeof SKELETON_ANIMATIONS;
...
export function Skeleton({
  ...
  const getSpritePath = () => {
    return `/sprites/enemies/skeleton/${animation}.png`;
  };
  ...
  const loadSpriteSheet = () => {
    ...
    image.onload = () => {
      setIsLoading(false);
      const animConfig = SKELETON_ANIMATIONS[animation];
      requestAnimationFrame((timestamp) => startAnimation(image, animConfig, timestamp));
    };

    image.onerror = () => {
      console.error('Failed to load Skeleton sprite:', spritePath);
