...
// White Werewolf specific animations and configurations
export const WHITE_WEREWOLF_ANIMATIONS = {
...
export type WhiteWerewolfAnimationType = keyof typeof WHITE_WEREWOLF_ANIMATIONS;

export interface WhiteWerewolfProps {
  animation?: WhiteWerewolfAnimationType;
  scale?: number;
  onAnimationComplete?: () => void;
  className?: string;
}

export function WhiteWerewolf({
  animation = 'Idle',
  scale = 1,
  onAnimationComplete,
  className = ''
  ...
  const getSpritePath = () => {
    return `/sprites/enemies/warewolfs/white_warewolfs/${animation}.png`;
  };
  ...
  useEffect(() => {
    ...
    const loadImage = async () => {
      ...
      try {
        ...
        if (isMounted) {
          setIsLoading(false);
          const animConfig = WHITE_WEREWOLF_ANIMATIONS[animation];
          requestAnimationFrame((timestamp) => startAnimation(image, animConfig, timestamp));
        }
      } catch (err) {
        if (isMounted) {
          console.error('Failed to load White Werewolf sprite:', spritePath);
