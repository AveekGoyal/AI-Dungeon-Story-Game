...
// Mountain Troll specific animations and configurations
export const MOUNTAIN_TROLL_ANIMATIONS = {
...
export type MountainTrollAnimationType = keyof typeof MOUNTAIN_TROLL_ANIMATIONS;

interface MountainTrollProps {
  animation?: MountainTrollAnimationType;
  scale?: number;
...
export function MountainTroll({
  ...
  useEffect(() => {
    ...
    const totalFrames = MOUNTAIN_TROLL_ANIMATIONS[animation].frames;

    const loadFrame = (index: number) => {
      return new Promise<void>((resolve, reject) => {
        const img = new Image();
        const paddedIndex = index.toString().padStart(3, '0');
        img.src = `/sprites/enemies/trolls/troll3/${animation}_${paddedIndex}.png`;
  ...
  useEffect(() => {
    ...
    const { frames, frameRate } = MOUNTAIN_TROLL_ANIMATIONS[animation];
