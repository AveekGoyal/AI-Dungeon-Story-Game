...
// Skeleton Warrior specific animations and configurations
export const SKELETON_WARRIOR_ANIMATIONS = {
  ...
  Attack_2: { frames: 6, frameRate: 12 },
  Attack_3: { frames: 4, frameRate: 8 },
  Run_Attack: { frames: 7, frameRate: 14 },
  Protect: { frames: 1, frameRate: 2 },
  Hurt: { frames: 2, frameRate: 4 },
  Dead: { frames: 4, frameRate: 8 }
} as const;

export type SkeletonWarriorAnimationType = keyof typeof SKELETON_WARRIOR_ANIMATIONS;

export interface SkeletonWarriorProps {
  animation?: SkeletonWarriorAnimationType;
  scale?: number;
...
export function SkeletonWarrior({
  ...
  const getSpritePath = () => {
    return `/sprites/enemies/skeleton_warrior/${animation}.png`;
  };
  ...
  useEffect(() => {
    ...
    const loadImage = async () => {
      ...
      try {
        ...
        if (isMounted) {
          setIsLoading(false);
          const animConfig = SKELETON_WARRIOR_ANIMATIONS[animation];
          requestAnimationFrame((timestamp) => startAnimation(image, animConfig, timestamp));
        }
      } catch (err) {
        if (isMounted) {
          console.error('Failed to load Skeleton Warrior sprite:', spritePath);
