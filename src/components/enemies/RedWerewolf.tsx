...
// Red Werewolf specific animations and configurations
export const RED_WEREWOLF_ANIMATIONS = {
...
export type RedWerewolfAnimationType = keyof typeof RED_WEREWOLF_ANIMATIONS;

export interface RedWerewolfProps {
  animation?: RedWerewolfAnimationType;
  scale?: number;
  onAnimationComplete?: () => void;
  className?: string;
}

export function RedWerewolf({
  animation = 'Idle',
  scale = 1,
  onAnimationComplete,
  className = ''
  ...
  const getSpritePath = () => {
    return `/sprites/enemies/warewolfs/red_warewolfs/${animation}.png`;
  };
  ...
  useEffect(() => {
    ...
    const loadImage = async () => {
      ...
      try {
        ...
        if (isMounted) {
          setIsLoading(false);
          const animConfig = RED_WEREWOLF_ANIMATIONS[animation];
          requestAnimationFrame((timestamp) => startAnimation(image, animConfig, timestamp));
        }
      } catch (err) {
        if (isMounted) {
          console.error('Failed to load Red Werewolf sprite:', spritePath);
